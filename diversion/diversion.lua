...
local pfunc = function()
    ...
    local upstreams = runtimeInfo.upstreams
    local runtimegroup = runtimeInfo.runtimegroup

    runtimeCache:setRuntime(hostname, divsteps, upstreams, runtimegroup)
    if red then
        setKeepalive(red)
    end
...
local ok, status, steps, upstreams, runtimeInfo = xpcall(pfunc, handler)
log:debug('divstep = ', steps,
        '\truntimeinfo = ', cjson.encode(runtimeInfo), '\t upstreams =', upstreams)
...
if upstream then
    ...
    local ngxUpstreams = utils.delArrayElement(ngx_upstream.get_upstreams(), utils.split(upstreams, ','))
    math.randomseed(utils.randomseed())
    local num = math.random(1, #ngxUpstreams)
    ngx.var.backend = ngxUpstreams[num]
end
log:info("============= upstream is ", ngx.var.backend)
