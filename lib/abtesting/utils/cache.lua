...
_M.getUpstreams = function(self, hostname)
    local cache = self.cache
    local k_upstreams = runtimeLib .. ':' .. hostname .. ':' .. fields.upstreams
    local upstreams = cache:get(k_upstreams)
    return tostring(upstreams)
end
...
_M.setRuntime = function(self, hostname, divsteps, upstreams, runtimegroup)
    local cache = self.cache
    local prefix = runtimeLib .. ':' .. hostname
    ...
    local k_upstreams = prefix .. ':' .. fields.upstreams
    local ok, err = cache:set(k_divsteps, divsteps, shdict_expire)
    if not ok then
        return false
    end
    local ok, err = cache:set(k_upstreams, upstreams, shdict_expire)
    if not ok then
        return false
    end
    return true
