...
---
-- private function, set diversion upstreams
-- @param id identify a policy
-- @param upstreams diversion upstreams (ipange/uid/...)
-- @return allways returned SUCCESS
_M._setDivupstreams = function(self, id, upstreams)
    local database = self.database
    local key = table.concat({ self.baseLibrary, id, fields.divupstreams }, separator)
    local ok, err = database:set(key, upstreams)
    if not ok then
        error { ERRORINFO.REDIS_ERROR, err }
    end
end
...
_M.set = function(self, policy)
    ...
    self:_setDivupstreams(id, policy.divupstreams)
...
_M.get = function(self, id)
    ...
    local divupstreamsKey = table.concat({ self.baseLibrary, id, fields.divupstreams }, separator)
    local database = self.database
    local policy = {}
    policy.divtype = ngx.null
    policy.divdata = ngx.null
    policy.divupstreams = ngx.null
    ...
    local divupstreams, err = database:get(divupstreamsKey)
    if not divupstreams then
        --add log
    end
    ...
    policy.divupstreams = divupstreams
    return policy
end
