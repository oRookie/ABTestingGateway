...
_M.set = function(self, domain, policyGroupId, divsteps)
    ...
    local policy = policyMod:get(group[1])
    local divupstreams = policy.divupstreams
    log:info("set policy group divupstreams is = ", divupstreams)
    ...
    local upStreams = prefix .. ':' .. fields.upstreams
    database:set(upStreams, divupstreams)
...
_M.get = function(self, domain)
    ...
    local upStreams = prefix .. ':' .. fields.upstreams
    local ok, err = database:get(upStreams)
    if not ok then
        log:info("---------------1")
        --error{ERRORINFO.REDIS_ERROR, err}
    end
    local upstreams = tostring(ok)
